...
def test_target_gap_is_calculated_correctly() -> None:
...
def test_target_gap_cannot_be_negative() -> None:
    result = calculate_target_gap(
        projection=65.0,
        target_rate=60.0,
    )
...
def test_forecast_years_are_2025_to_2027() -> None:
...
def test_forecast_above_100_raises_error() -> None:
    dataframe = pd.DataFrame(
        {
            "year": [2025],
            "pessimistic": [49.5],
